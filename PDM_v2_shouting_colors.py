...
class CTkSpinbox(ctk.CTkFrame):
	def __init__(self, *args, command=None, values=None, **kwargs):
		...
		self.subtract_button = ctk.CTkButton(self, text="<<", width=10, height=10, command=self.subtract, fg_color="transparent") #, font=("cardana", 10 ))
		self.subtract_button.grid(row=0, column=0)#, padx=(0,2))

		self.entry = ctk.CTkEntry(self, width=30, height=10, justify="center", border_width=0, font=("cardana", 15 ), fg_color="transparent")
		self.entry.grid(row=0, column=1, padx=0.1)
		self.entry.insert(0, values[0] if values else "0")
		self.entry.bind("<Return>", lambda event: self.get_input(event))
		
		self.add_button = ctk.CTkButton(self, text=">>", width=10, height=10, command=self.add, fg_color="transparent") #, font=("cardana", 10))
		self.add_button.grid(row=0, column=2)#, padx=(2, 0))

	def add(self):
		if self.values:
			self.index = min(len(self.values) - 1, self.index + 1)
	...
	def get(self):
		...
		if self.values:
			if user_input in self.values:
				self.index = self.values.index(user_input)
				return user_input
			else:
				fallback = self.values[self.index]
				self.entry.delete(0, "end")
				self.entry.insert(0, fallback)
				return fallback
...
class DrumMachine:
	def __init__(self, root):
		...
		"""self.current_pattern = IntVar()
								...
								self.bpu = IntVar()"""
		self.mainFrame = ctk.CTkFrame(self.root, fg_color=BG_MAIN)
	...
	def display_button_color(self, row, col):
		...
		#button_color   = ACCENT_MINT if self.get_button_value(row, col) else original_color
	...
	def create_top_bar(self):
		topbar_frame = ctk.CTkFrame(self.mainFrame, height = 25, fg_color=BG_SUB, corner_radius=8)
		topbar_frame.grid(row = 0, columnspan = 12, rowspan=10, padx=5, pady=5, sticky="ew")

		# Pattern Number section
		ctk.CTkLabel(topbar_frame, text="Pattern Number:", text_color=TEXT_COLOR).grid(row=0, column=1, padx=5)
		self.pattern_index_widget = CTkSpinbox(topbar_frame, values=[str(i) for i in range(1, MAX_NUMBER_OF_PATTERNS + 1)],
										 command=self.on_pattern_changed)
		self.pattern_index_widget.grid(row=0, column=2, padx=10, pady=10)
		...
		ctk.CTkLabel(topbar_frame, text="Number of Units:", text_color=TEXT_COLOR).grid(row=0, column=4)
		...
		# BPU section
		ctk.CTkLabel(topbar_frame, text="BPUs:", text_color=TEXT_COLOR).grid(row=0, column=6)
	...
	def create_play_bar(self):
		...
		# Play Button
		self.play_button = ctk.CTkButton(playbar_frame, text="", fg_color=COLOR_1, hover_color=BUTTON_CLICKED_COLOR,
		...
		# Stop Button
		self.stop_button = ctk.CTkButton(playbar_frame, text="", width=10, fg_color=COLOR_1, hover_color=BUTTON_CLICKED_COLOR,
		...
		for row in range(MAX_NUMBER_OF_DRUM_SAMPLES):
			for col in range(self.find_number_of_columns()):
				self.buttons[row][col] = ctk.CTkButton(
					...
					text="", height=35, width=20, hover_color=HOVER)
	...
	def create_top_menu(self):
		...
		self.menu_frame = ctk.CTkFrame(self.root, height=40, corner_radius=0, fg_color=BG_SUB)
		self.menu_frame.pack(side="top", fill="x")

		file_menu = ctk.CTkOptionMenu(self.menu_frame, values=["Load Project", "Save Project", "Exit"], command=self.handle_file_menu,
								width=100, height=25)
		file_menu.set("File")
		file_menu.pack(side="left", padx=10, pady=5)

		about_btn = ctk.CTkButton(self.menu_frame, text="About", width=80, height=25, fg_color="transparent", hover_color=PAD_OFF_2,
							command=self.show_about)
		about_btn.pack(side="left", padx=5)

	def handle_file_menu(self, choice):
		if choice == "Load Project": self.load_project()
		elif choice == "Save Project": self.save_project()
		elif choice == "Exit": self.exit_app()
